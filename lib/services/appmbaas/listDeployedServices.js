...
module.exports = function listDeployedServices(params, cb){
  ...
  log.logger.debug("listDeployedServices ", params);

  if(!params.domain || !params.environment){
    log.logger.error("listDeployedServices: Invalid Parameters, ", {domain: !params.domain, environment: !params.environment});
  ...
  fhMbaasMiddleware.appmbaas().find({domain: params.domain, environment: params.environment, isServiceApp: true}, function(err, serviceApps){
    if(err){
      log.logger.error("Error Getting Deployed Services ", {error: err, params: params});
    }
