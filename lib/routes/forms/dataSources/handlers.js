...
function getServiceData(dataSource, deployedService, cb){
  ...
  log.logger.debug("deployedService", {
  ...
  if(!dataSourceUpdater){
    dataSourceUpdater = dataSourceUpdaterModule(log.logger);
  }
  ...
  dataSourceUpdater.handlers.requestEndpointData({
    ...
    if(err){
      log.logger.error("Error Getting Service Data ", {error: err});
    ...
    if(!serviceData){
      log.logger.error("No Service Data Passed ");
      dataSource.validationResult = {
        valid: false,
        message: err.userDetail
      };
...
function deploy(req, res, next){
  log.logger.debug("Deploy Data Source ", req.body);
  ...
  async.waterfall([
    ...
    if(err){
      log.logger.error("Error Deploying Data Source ", {error: err});
      return next(err);
    }

    log.logger.debug("Finished Deploying Data Source", deployedDataSource);
...
function getDSDeployedService(req, dataSource, cb){
  ...
  log.logger.debug("getDSDeployedService ", dataSource);
  ...
  getDeployedService({
    ...
    log.logger.debug("getDSDeployedService ", deployedService);
...
function validate(req, res, next){
  ...
  log.logger.debug("Validate Data Source", {
    params: req.params,
    dataSource: req.body
  });
  ...
  async.waterfall([
    ...
    function validateDataSource(serviceData, cb){
      log.logger.debug("Calling Validate ", {
        uri: req.mongoUrl
      }, dataSource);
    ...
    if(err && err !== dataSource){
      log.logger.error("Error Validating Data Source", err);
      return next(err);
    }

    log.logger.debug("Validation Result ", validationResult || dataSource);
...
function getAuditLogEntry(req, res, next){
  async.waterfall([
    ...
    log.logger.debug("Responding With Audit Logs");
