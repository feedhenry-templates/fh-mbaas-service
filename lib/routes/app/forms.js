...
router.post('/submissions/:id/complete', fhForms.middleware.submissions.completeSubmission, function(req, res, next){
  fhmbaasMiddleware.appformsMiddleware.notifySubmissionComplete(req, res, function(err){
    if(err){
      log.logger.error('Failed to notify core platform submission complete', err);
    } else {
      log.logger.debug('ubmission notification complete');
    }
  });
  return next();
});
...
router.use(function(err, req, res, next){//jshint unused:false
  log.logger.error('Error Performing Forms Operation ', err);
